...
# --- 測站經緯度資訊 ---
STATION_COORDS = {
    "ESPRO": {"latitude": 0.0, "longitude": 0.0, "elevation": 0.0},
    "KHH01": {"latitude": 22.6, "longitude": 120.3, "elevation": 50.0},
    "NWT01": {"latitude": 24.1, "longitude": 120.6, "elevation": 0.0},
}
# -------------------------

station = os.getenv("station", "ESPRO")

def fetch_data(mysql: mysql_connector, start_time_ms, end_time_ms, station_name, data_type='raw', tz_utc_8=timezone(timedelta(hours=8))):
    """從資料庫獲取指定時間範圍內的資料"""
...
def export_to_miniseed(sensor_rows, station_info, output_file='seismic_data.mseed', station_name=station, save=True):
    ...
    for channel, data in components:
        ...
        # 將經緯度資訊加入 Trace 的 stats 中
        # if station_info:
        #     stats['stla'] = station_info['latitude']
        #     stats['stlo'] = station_info['longitude']
        #     stats['stel'] = station_info['elevation']
...
def main():
    ...
    # start_time 自動定出前後 args.time 分鐘為開始與結束時間，優先級比 start_time 和 end_time 高
